 ...
 */
public class CreateStudentsTable implements Migration {

    @Override
    public void up(Database database) {
        database.create("students", table -> {
            table.integer("student_id").incremented();
            table.string("credential", 7);
            table.string("first_name", 64);
            table.string("last_name", 64);

            table.primary("student_id");
            table.unique("credential");
        });
    }

    @Override
    public void down(Database database) {
        database.drop("students");
    }
}
